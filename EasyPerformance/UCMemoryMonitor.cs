...
namespace CloudBox.Performance
{
    public partial class UCMemoryMonitor : UserControl
    {
        ...
        {
            lblCurrent.Text = PerformanceDetector.Instance.MainMemory.GetCurrentKB();
            lblMax.Text = PerformanceDetector.Instance.MainMemory.GetMaximumKB();

            if (m_isFirst)
            {
                lsvModule.Items.Clear();
                foreach (var mem in PerformanceDetector.Instance.ModuleMemorys.Values)
                {
